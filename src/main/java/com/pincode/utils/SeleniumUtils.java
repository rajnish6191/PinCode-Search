...
public class SeleniumUtils {
	...
	/**
	 * Try to start working from setted point
	 *  
	 * @param driver
	 * @param id
	 * @param searchPoint
	 * @return
	 */
	public List<String> labelsFromWebElementWithSearchPoint(WebDriver driver, String id, String searchPoint){
		WebElement element = driver.findElement(By.id(id));
		Select select = new Select(element);
		List<WebElement> options = select.getOptions();
		...
		if(searchPoint.isEmpty()){
			...
			for (WebElement option : options) {
				if(!option.getText().trim().equals("Select Locality")){
					if(option.getText().trim().equals(searchPoint)){
						elementFinded = true;
					}
					if(elementFinded){
						labels.add(option.getText());
